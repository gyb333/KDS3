...
namespace Wilmar.Build.Core.Dojo.Default.Builders
{
    ...
    /// 按钮生成器
    /// </summary>
    public class ButtonBuild : ControlBuildBase
    {
        public ButtonBuild(bool isPreview, ControlHost controlHost, ScreenDefinition screenDef, CompileBase compile, ProjectDocument doc, Dictionary<int, Tuple<int, string>> permissionData, HtmlTextWriter htmlWriter)
        ...
        {
            Button control = this.ControlHost.Content as Button;
            if (control.IsBusy)
            {
                this.HtmlWriter.AddAttribute("dojoType", "Controls/BusyButton");
                if (!string.IsNullOrEmpty(control.BusyLabel))
                {
                    this.HtmlWriter.AddAttribute("busyLabel", control.BusyLabel);
                }
            }
            else this.HtmlWriter.AddAttribute("dojoType", "Controls/Button");
